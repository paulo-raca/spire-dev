...
- **SLSA build provenance**, signed via Sigstore using the GitHub Actions workflow identity (keyless, no long-lived keys). Generated by [`actions/attest-build-provenance`](https://github.com/actions/attest-build-provenance) and attached as an OCI 1.1 referrer to the image manifest — no `.sig` sibling tags in the tag list.
