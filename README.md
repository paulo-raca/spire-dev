...
- **Workflow-side SLSA provenance (v1.0)** — generated by [`actions/attest-build-provenance`](https://github.com/actions/attest-build-provenance), signed via Sigstore using the GitHub Actions workflow identity (keyless, no long-lived keys). Describes the source repo, commit, workflow file, and run.
- **BuildKit-side SLSA provenance (v0.2, `mode=max`)** — generated by `docker/build-push-action`'s native provenance. Describes the build inputs, layer steps, and BuildKit cache state.
- **SBOM** — Software Bill of Materials attached as an OCI artifact alongside the image.

All three are attached as OCI 1.1 referrers to the image manifest — no `.sig` sibling tags clutter the tag list.
...
```bash
cosign verify-attestation paulocosta56/spire-dev:latest \
  --type slsaprovenance \
