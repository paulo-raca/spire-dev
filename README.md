...
- **Workflow-side SLSA provenance (v1.0)** — generated by [`actions/attest-build-provenance`](https://github.com/actions/attest-build-provenance), signed via Sigstore using the GitHub Actions workflow identity (keyless, no long-lived keys). Describes the source repo, commit, workflow file, and run. Indexed in GitHub's attestation store (verifiable per-image via the GH CLI; not attached as an OCI referrer due to a Docker Hub auth interop bug in the action — see `ci.yml`).
- **BuildKit-side SLSA provenance (v0.2, `mode=max`)** — generated by `docker/build-push-action`'s native provenance. Describes the build inputs, layer steps, and BuildKit cache state. Attached as an OCI 1.1 referrer.
- **SBOM** — Software Bill of Materials attached as an OCI artifact alongside the image.

Verify the workflow-side attestation with the GitHub CLI:

```bash
gh attestation verify oci://paulocosta56/spire-dev:latest \
  --repo paulo-raca/spire-dev
```

Inspect the BuildKit-side provenance and SBOM via:

```bash
docker buildx imagetools inspect paulocosta56/spire-dev:latest --format '{{ json .Provenance }}'
docker buildx imagetools inspect paulocosta56/spire-dev:latest --format '{{ json .SBOM }}'
```
