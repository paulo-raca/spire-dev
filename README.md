...
- **Cosign keyless signature** — signed via Sigstore using the GitHub Actions workflow OIDC identity (no long-lived keys). Stored as an OCI 1.1 referrer to the image manifest — no `.sig` sibling tags.
...
Inspect the BuildKit-side provenance and SBOM:
